...
object Hangman extends App {
  ...
  val chooseWord: IO[IOException, String] =
    for {
      rand <- nextInt(Dictionary.length)
      theLift: Option[String] = Dictionary.lift(rand)
    } yield theLift.getOrElse("Bug in the program!")
  ...
  def gameLoop(state: State): IO[IOException, State] = {
    for {
      ...
      _ <- renderState(state)
      loop <- state match {
        case hasWon if hasWon.playerWon                        =>
          putStrLn(s"Congratulations ${state.name} you won the game!").const(false)
        case hasLost if hasLost.playerLost                     =>
          putStrLn (s"Sorry ${state.name} you lost the game. The word was ${state.word}").map(_ => false).const(false)
        case stillPlaying if stillPlaying.word.contains(guess) =>
          putStrLn (s"You guessed correctly!").const(true)
        case _                                                 =>
          putStrLn(s"That's wrong. but keep trying!").const(true)
      }
